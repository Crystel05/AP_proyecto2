...
app.get("/logIn/:correo", async(req, res)=>{
    ...
    try{
        const contra = await pool.query("SELECT \"ID\",contrasenna, nombre, apellido, cedula FROM usuario WHERE correo = $1", [correo]);
...
//Log in, ENTRADAS: El ID del usuario SALIDA: El tipo de usuario
...
//Dar todos los cursos a Profesores con su correo, ENTRADAS: el correo del profesor SALIDA: los cursos del profesor con ese correo
...
app.get("/CedPorCurso/:cod/:grad", async(req, res)=>{
    const { cod } = req.params
    const { grad } = req.params
    try{
        const profesor = await pool.query("SELECT * FROM obtenercedprof($1,$2)" , [cod,grad]);
        res.json(profesor.rows)
...
app.get("/estudiantes/:nom/:apel", async(req, res)=>{
    ...
    try{
        const estudiante = await pool.query("SELECT nombre, cedula, contrasenna, apellido, clase, correo FROM estudiante INNER JOIN usuario ON estudiante.\"usuarioId\" = usuario.\"ID\" INNER JOIN grado ON estudiante.\"gradoId\" = grado.\"ID\" WHERE nombre = $1 AND apellido = $2", [nom, apel]);
...
app.get("/noticias/:codigo/:clase", async(req, res)=>{
    ...
    const { clase } = req.params
    try{
        const noticia = await pool.query("SELECT * FROM noticia INNER JOIN curso ON curso.\"ID\" = \"cursoId\" INNER JOIN grado ON grado.\"ID\" = \"gradoId\" WHERE curso.codigo = $1 AND grado.clase = $2", [codigo, clase]);
...
app.get("/tareas/:codigo/:clase", async(req, res)=>{
    ...
    const { clase } = req.params
    try{
        const tarea = await pool.query("SELECT * FROM tarea INNER JOIN curso ON curso.\"ID\" = \"cursoId\" INNER JOIN grado ON grado.\"ID\" = \"gradoId\" WHERE curso.codigo = $1 AND grado.clase = $2", [codigo, clase]);
        res.json(tarea.rows)
...
app.get("/chat/:codigo/:clase", async(req, res)=>{
    ...
    const { clase } = req.params
    try{
        const mensajes = await pool.query("SELECT usuario.nombre, apellido, texto FROM mensaje INNER JOIN chat ON chat.\"ID\" = \"chatId\" INNER JOIN curso ON curso.\"ID\" = \"cursoId\" INNER JOIN grado ON grado.\"ID\" = curso.\"gradoId\" INNER JOIN usuario ON usuario.\"ID\" = mensaje.\"usuarioId\" WHERE codigo = $1 AND clase = $2 ORDER BY mensaje.\"ID\" DESC", [codigo, clase]);
...
app.get("/publicaMsg/:curso/:grado/:correo/:mensaje", async(req, res)=>{
    const { curso } = req.params
    const { grado } = req.params
    const { correo } = req.params
    const { mensaje } = req.params
    try{
        const mensajes = await pool.query("SELECT * FROM insertarmensaje($1, $2, $3, $4)", [curso, grado, correo, mensaje]);
        res.json(mensajes.rows)
...
app.get("/votarNota/:cedula/:nuevanota", async(req, res)=>{
    ...
    try{
        const nota = await pool.query("SELECT * FROM actualizarnotadocente($1, $2)", [cedula, nuevanota]);
        res.json(nota.rows)
